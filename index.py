...
@client.command(name= "ping", description = "Plus c'est haut plus t'es retardé", visibilty = True)
async def ping(ctx):
...
@client.command(pass_context = True , name = "help", description = "T'envoie mon aide", visibilty = False)
async def help(ctx, *help_type):
    ...
    for command in client.commands:
        if command.name not in not_see_command:
...
@client.command(name = "clear", description = "Permet supprimer des messages", type = "administration", visibilty = True)
async def clear(ctx, amount=5):
    await ctx.channel.purge(limit=amount)


@client.command(name = "configserver", description="Sauvegarde la configuration du serveur discord", visibilty = True)
...
@client.command(name = "deleteserver", description = "Suprimme la sauvegarde du serveur", visibilty = True)
...
@client.command(name = "rewriteserver", description = "Recréé le serveur à partir de sa sauvegarde", visibilty = True)
